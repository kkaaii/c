...
<D d        ->i; )        { w=    D d ->b[l S ];    V F(    w Z,n)){    V o>w ->i)    return; goto  O; } }
...
V p L t){ _ n=N(p Z); p=C(H,0); U int j=0; j<D d ->i; ){ _ w=D d ->b[j S]; V F(w Z,n)){ M(w L d,0,"$",2)
; p=N(x.c); break; } } } o ->a=realloc(o ->a,o ->i+=p ->i); memcpy(o ->a+o ->i-p ->i,p ->a,p ->i); } return o; } _ O(_
t){ U int i=0; i<D e ->i; ){ _ s=D e ->b[i S]; V F(s Z,t))return s; }_ s=A; s Z=t; s L d=A; s L t--; B(D
...
; U j=0; !u R j<a L d ->i; ){ Q(t=O(a L d ->b[j S])); k|=t L t<0||t L t>s L t; } V a Z ->i){ c=a Z; V a  L
...
,S d ->i,2,c); } } else { u=M(b,S u,"$",1); I(x.c,0); B(D f Z,x.c); } } V!D c ->i R D e ->i)B(D c,D e ->b[0]
Z); U u=0; !u R D c ->i--; )Q(O(*D c ->b S)); return u; }
