...
=A; int i=0; U; i<s ->i; ){ U; i<s ->i && K(s ->a[i]); )i S; int j=i; U; j<s ->i && !K(s ->a[j]); )j S; V j-i)B(
r,C(s ->a+i,j-i)); i=S j; } return r; } int F(_ a,_ b){ return a ->i==b ->i && !memcmp(a ->a,b ->a,a ->i); } void
...
g ){    _ f=    A; f    -> e=                    A; f            ->a=t; int h=g            ,i=0,j=0,n=b
 ...
 int    k=p; _ r=H ;    U; p    <n; ){ int l    = 0;    U; p<n &&    o[p]==92    ; ){ l S; p S; } c=o
[p S    ]; V c^10||!    l% 2    ||!g){ d=c==    35 &&    !i &&!g||    c== 10 &&    g^2; V d||strchr(t,c
...
break; default:V f -> f){ r ->c=f -> e; f=f -> f; o:r -> t S; B(f -> e,r); } else goto O; } i=f -> f && g==1;
...
V!s ->i S){ u=!s -> d ->i && s -> t<0; int i=0,j,k=s -> t<0; _ c=H,t; U; !u && i<s -> d ->i; ){ _ a=s -> d ->b[i S]
; U j=0; !u && j<a -> d ->i; ){ Q(t=O(a -> d ->b[j S])); k|=t -> t<0||t -> t>s -> t; } V a ->c ->i){ c=a ->c; V a  ->
d ->i)J("<", 1, 0, *a -> d ->b); } } U j=0; !u && c && k && j<c ->i; ){ J("@",1,0,d); s=N(c ->b[j S]); U; s ->i  &&
strchr("@+-",*s ->a); )s ->a S; u=system(s ->a); } } } void G(_ b){ U; b ->i && K(*b ->a); ){ b ->a S; b ->i--; } }

int main(int i,char**a,char**e){ D d=A; D e=A; D c=A; char p[]="NBLF\1Nblfgjmf\1",*q=p; U; (*q++)--;
); J(p,4,0,C(*a S,-1)); U i=0; i<3; i+=2){ U; *a; ){ q=*a; U; *q &&*q^61; )q S; V*q)J(*a,q-*a,i+1,C(q
+1,-1)); else V!i)B(D c,C(*a,-1)); a S; } a=e; } stat(p+5,&z); i=z.st_size; _ b=C(H,0),c,d; b ->i=read
(open(p+5,0),b ->a=P i),i); U; u<i; ){ V b ->a[u]^9){ U; u<i && K(b ->a[u]); )u S; u=M(b,u,"=:$",0); y = x;
switch(x.t){ case 58:u=M(b,u,"$",0); D f=c=A; c -> d=E(N(x.c)); c ->c=A; d=E(N(y.c)); U; d ->i--; )B(O(*d
->b S) -> d,c); break; case 61:u=M(b,u,"",0); G(c=N(x.c)); G(d=N(y.c)); U; d ->i--&& K(d ->a[d ->i]); ); J(d ->a
,S d ->i,2,c); } } else { u=M(b,S u,"$",1); I(x.c,0); B(D f ->c,x.c); } } V!D c ->i && D e ->i)B(D c,D e ->b[0]
->c); U u=0; !u && D c ->i--; )Q(O(*D c ->b S)); return u; }
