...
static void Log30h_Page02h_Capacity(void *buf)
{
...
static void Log30h_Page03h_Supported_Capabilities(void *buf)
{
	uint64	*entry = (uint64 *)buf;
	...
	*entry++ = QWORD_VALID | ATA_Field(NOMINAL_MEDIA_ROTATION_RATE);

	/* 32..39 - Form Factor */
	*entry++ = QWORD_VALID | ATA_Field(NOMINAL_FORM_FACTOR);

	/* 40..47 - Write-Read-Verify Sector Count Mode 3 */
	*entry++ = QWORD_VALID | ATA_Field(WRV_MODE3_COUNT);

	/* 48..55 - Write-Read-Verify Sector Count Mode 2 */
	*entry++ = QWORD_VALID | ATA_Field(WRV_MODE2_COUNT);

	/* 56..71 - World wide name */
	ATA_Copy((void *)entry, ATA_Field(WORLD_WIDE_NAME), 8);
	++entry;
	*entry++ = QWORD_VALID;
	...
	*entry++ = QWORD_VALID | ATA_Field(ZONED);
	...
	*entry++ = QWORD_VALID |
	...
	/* 136..143 - Queue Depth */
	*entry++ = QWORD_VALID | ATA_Field(QUEUE_DEPTH);
...
static void Log30h_Page04h_Current_Settings(void *buf)
{
	uint64	*entry = (uint64 *)buf;
...
static void Log30h_Page05h_Strings(uint8 *buf)
{
	uint64	*entry = (uint64 *)buf;
	...
	*entry++ = QWORD_VALID | (PAGE_NUMBER_05H << 16) | REVISION_NUMBER;
	...
	/* 28..31 Reserved */
	ATA_Copy(buf + 8, ATA_Field(SERIAL_NUMBER));

	/* 32..39 - FIRMWARE REVISION field */
	/* 40..47 Reserved */
	ATA_Copy(buf + 32, ATA_Field(FIRMWARE_REVISION));

	/* 48..87 - MODEL NUMBER field */
	/* 88..95 Reserved */
	ATA_Copy(buf + 48, ATA_Field(MODEL_NUMBER));

	/* 96..103 - ADDITIONAL PRODUCT IDENTIFIER field */
	/* 104..511 Reserved */
	ATA_Copy(buf + 96, ATA_Field(ADDITIONAL_PRODUCT_IDENTIFIER));
}

static void Log30h_Page06h_Security(void *buf)
{
	uint64	*entry = (uint64 *)buf;
...
static void Log30h_Page07h_Parallel_ATA(void *buf)
{
	uint64	*entry = (uint64 *)buf;
...
static void Log30h_Page08h_Serial_ATA(void *buf)
{
	uint64	*entry = (uint64 *)buf;
